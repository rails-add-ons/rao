...
RSpec.describe "/foos", type: :feature do
  let(:base_path) { "/foos" }

  around(:each) do |example|
    Rails.application.routes.draw { resources :foos }
    example.run
  ...
  before(:each) {
    foos_controller = Class.new(Rao::ResourcesController::Base) do
    ...
    stub_const('FoosController', foos_controller)
  }
  ...
  describe 'REST actions' do
    ...
    describe 'create' do
      describe 'when valid' do
        ...
        xit { expect(current_path).to eq(after_create_path) }
        xit { expect(page).to have_http_status(200) }
      end
    ...
    describe 'update' do
      ...
      describe 'when valid' do
        ...
        xit { expect(current_path).to eq(after_update_path) }
        xit { expect(page).to have_http_status(200) }
      end
